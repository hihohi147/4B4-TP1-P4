...
public class Facture {
	...
	public static void AffichageFinal(double prix) {

		System.out.println("Prix : " + setFormatMonetaire(prix) + "$" + " + Taxes ("
				+ taxeEnString(calculerTaxe(prix)) + "$) = " + prixTotalEnString(calculerPrixT(prix, TAXES)) + "$\n");
	}
...
return prixTotal;
	...
	public static String prixTotalEnString(double prixTotal) {
		String prixTotalEnString;
		if (prixTotal>=0) {
			 prixTotalEnString = setFormatMonetaire(prixTotal);

			}else {
				prixTotalEnString = "Doit être supérieur à 0";
			}
			...
			try {
				...
				for (int i = 1; i < nbrClient; i++) {
					...
					if (prix != 0.0) {
						...
						w.write("Factures : \n");
						w.newLine();
						w.write(tabClients[i] + " : Prix : " + setFormatMonetaire(prix) + "$" + " + Taxes ("
								+ taxeEnString(calculerTaxe(prix)) + "$) = "
